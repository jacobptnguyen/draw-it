...
const ChatBot = () => {
  ...
  const { sendDrawingForFeedback, compareWithReference, isLoading } = useAI();
  ...
  useEffect(() => {
    const autoGenerateDailyChallenge = async () => {
      if (
        ...
        !isLoading && // Wait for messages to finish loading
        ...
        if (!hasChallengeContent && !hasGeneratedDailyChallenge) {
          try {
            ...
            // Use the thumbnail_url from the database
            const referenceImageUrl = todaysChallenge.thumbnail_url;
            
            if (!referenceImageUrl) {
              throw new Error('No reference image found in database challenge');
            }
            
            // Add the challenge description as AI message
            await addMessage(challengeText, 'ai', 'text');
            
            // Add the reference image as AI message with proper text
  ...
  }, [session?.id, routeDrawingId, isLoading, messages, hasGeneratedDailyChallenge, todaysChallenge]);
  ...
  const handleUploadImage = async () => {
    if (!session || isLoading || isUploading) return;
        ...
        >
          ...
          <View style={styles.inputContainer}>
            ...
            <TextInput
              ...
              editable={!isLoading && !isUploading && canSendMessage}
              multiline
              returnKeyType="send"
            ...
            <View style={styles.inputRow}>
              <View style={styles.leftButtons}>
                <TouchableOpacity
                  ...
                  disabled={isLoading || isUploading || !canSendMessage}
                  style={[
                    styles.actionButton,
              ...
              <TouchableOpacity
                ...
                disabled={!input.trim() || isLoading || isUploading || !canSendMessage}
                style={[
                  styles.actionButton,
                  (!canSendMessage) && styles.disabledButton
                ]}
